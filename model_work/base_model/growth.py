...
def scale_demand(demand_2024_mt, commodity, annual_growth_rate=None, years=4):

    defaults = {"steel": 0.009, "fertilizer": 0.0142, "shipping": 0.0254}  # ~0.9% for steel, ~1.42% for fertilizer, ~2.55% for shipping

    commodity = commodity.lower()
    if commodity not in defaults:
        raise ValueError("Commodity must be 'steel' or 'fertilizer' or 'shipping'.")
...
shipping = demand.loc[demand["industry"] == "shipping", "demand_mt"].values[0]

print(scale_demand(steel, "steel"))              # ~1858.9 Mt
print(scale_demand(fertilizer, "fertilizer"))
   # ~208.1 Mt
print(scale_demand(shipping, "shipping"))

scaled_demand = pd.DataFrame({
    "industry": ["steel", "fertilizer","shipping"],
    "demand_mt": [scale_demand(steel, "steel"), scale_demand(fertilizer, "fertilizer"), scale_demand(shipping, "shipping")]
})
print(scaled_demand)
