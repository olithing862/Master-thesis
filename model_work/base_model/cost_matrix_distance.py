...
nodes = pd.read_csv("model_work/DataFiles_base/nodes.csv").set_index('node_id')
print("Dtypes:")
print(nodes.dtypes)

print("\nDuplicate node_ids (in index):")
print(nodes[nodes.index.duplicated(keep=False)])

print("\nNon-numeric lat values:")
print(nodes[pd.to_numeric(nodes['lat'], errors='coerce').isna()])

print("\nNon-numeric lon values:")
print(nodes[pd.to_numeric(nodes['lon'], errors='coerce').isna()])
# --- Distance-to-cost interpolation (onshore) ---
distance_cost_df = pd.read_csv("model_work/DataFiles_base/onshore_cost.csv")
interp_cost = interp1d(distance_cost_df['distance'],
                       distance_cost_df['cost'],
                       fill_value='extrapolate')

# --- Sea cost matrix (T -> T, region-based) ---
sea_cost = pd.read_csv("model_work/DataFiles_base/shippingcost.csv", index_col='Cluster')
...
unavailable   = pd.read_csv("model_work/DataFiles_base/illegalconnection.csv")
transit_only  = pd.read_csv("model_work/DataFiles_base/transit_only_production.csv")
...
c.to_csv("model_work/DataFiles_base/cost_matrix_5.csv")